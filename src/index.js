...
const TableRenderer = (options = {}) => {
  ...
  const getTableHeight = (title, columns, dataSource) => {
    ...
    const bodyHeight = (_dataSource$reduce = dataSource === null || dataSource === void 0 ? void 0 : dataSource.reduce((height, row) => height + (row === '-' ? 1 : Math.max(...Object.values(row).map(content => (content.toString().match(/\n/g) || []).length)) * 20 + cellHeight), 0)) !== null && _dataSource$reduce !== void 0 ? _dataSource$reduce : 0;
  ...
  const renderTable = ({
    ...
    const info = {
      ...
      y: new Array((_dataSource$length = dataSource === null || dataSource === void 0 ? void 0 : dataSource.length) !== null && _dataSource$length !== void 0 ? _dataSource$length : 0).fill().map((_, i) => {
        const titleHeight = title ? cellHeight + titleSpacing : 0;
        const headerHeight = !(columns === null || columns === void 0 ? void 0 : columns.length) || columns.every(col => !col.title) ? 0 : cellHeight;
        return top + titleHeight + headerHeight + (dataSource === null || dataSource === void 0 ? void 0 : dataSource.reduce((y, row, j) => y + (j >= i ? 0 : row === '-' ? 1 : Math.max(...Object.values(row).map(content => (content.toString().match(/\n/g) || []).length)) * 20 + cellHeight), 0));
      })
    };
